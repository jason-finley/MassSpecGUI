...
class TImage(GraphicsObject):
    ...
    def __init__(self, p, *pixmap):
        GraphicsObject.__init__(self, [])
        self.anchor = p.clone()
        self.imageId = TImage.idCount
        TImage.idCount = TImage.idCount + 1
        if type(pixmap[0]) == str: # file name provided
            self.img = tk.PhotoImage(file=pixmap[0], master=_root)
        else: # PIL.ImageTk.PhotoImage provided
            self.img = pixmap[0]
